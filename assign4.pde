...
void setup() {
  ...
  for(int f = 0; f < 8; f++){
    for(int g = 8; g < 16; g++){
      if(int(f % 4) == 0 || int(f % 4) == 3){
        if(int(g % 4) == 1 || int(g % 4) == 2){
          soilHealth[f][g] = 30;
          }
        }
    
      if(int(f % 4) == 1 || int(f % 4) == 2){
        if(int(g % 4) == 0 || int(g % 4) == 3){
          soilHealth[f][g] = 30;
        }
      }
    }
  ...
  for(int i = 1; i < 24; i++){
    ...
    for(int j = 0; j < hole; j++){
      soilHealth[floor(random(SOIL_COL_COUNT))][i] = 0;
    }
  }
...
void draw() {
  ...
  switch (gameState) {
    ...
    image(soldier,soldierX[i] - SOLDIER_H,soldierY[i]*80+i*320);
      if(playerX + GROUNDHOG_H > soldierX[i] - SOLDIER_H && playerX < soldierX[i]
        && playerY + GROUNDHOG_H > soldierY[i]*80+i*320 && playerY < soldierY[i]*80 + i*320 + SOLDIER_H){
            playerX = PLAYER_INIT_X;
            playerY = PLAYER_INIT_Y;
            playerCol = (int) (playerX / SOIL_SIZE);
            playerRow = (int) (playerY / SOIL_SIZE);
